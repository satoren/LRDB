...
TEST_F(DebugServerTest, ConnectTest2) {
  ...
  std::thread client([&] {
    ...
    picojson::value res =
        sync_request(0, "add_breakpoint", picojson::value(break_point));
    res = sync_request(3, "get_breakpoints");
    // std::cout << res.serialize() << std::endl;

    sync_request(3, "continue");
    wait_for_paused();

    res = sync_request(3, "get_local_variable");
    //		std::cout << res.serialize() << std::endl;
    res = sync_request(3, "get_upvalues");
    //		std::cout << res.serialize() << std::endl;
    ...
    res = sync_request(3, "get_local_variable", stack_var_req_param);
    //		std::cout << res.serialize() << std::endl;
    res = sync_request(3, "get_upvalues", stack_var_req_param);
    //		std::cout << res.serialize() << std::endl;

    sync_request(4, "continue");
  });
